...
- 역할
    ...
    - 인코더 장비에서 송출되는 스트림을 웹에서 시청이 가능하도록 HLS로 변환하는 기능 개발

    #### HLS(HTTP Live Streaming) : HTTP기반으로 작은 덩어리 단위의 스트림을 통신하는 수단, 대중적인 스트리밍 포맷으로 사용된다. 
...
- 역할
    ...
    - 지도를 활용하여 사용자가 그린 다각형, 원 범위 안에 속해있는 업소들의 정보 표시하는 기능 개발
      ...
      - 조회한 업소 데이터들을 지도에 Marker로 각 업소들에 대한 정보를 출력
